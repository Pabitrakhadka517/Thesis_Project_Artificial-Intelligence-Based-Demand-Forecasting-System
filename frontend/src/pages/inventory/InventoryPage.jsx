...
import {
  ...
  AlertTriangle, CheckCircle, XCircle, TrendingDown, Activity,
} from 'lucide-react'
...
function AdjustModal({ product, onClose }) {
  ...
  const formId = `adjust-stock-form-${product._id}`
  ...
  return (
    <Modal
      ...
      title={
        <span>
          Adjust Stock
          <span className="block text-[12px] font-normal mt-0.5" style={{ color: 'var(--text-muted)' }}>{product.name}</span>
        </span>
      }
      footer={
        <Button type="submit" form={formId} className="w-full" loading={mutation.isPending}>
    ...
    >
      ...
      <form id={formId} onSubmit={handleSubmit(mutation.mutate)} className="space-y-4" noValidate>
        {/* Type selector */}
        <div>
          <span id="adjust-type-label" className="block text-[11px] font-semibold uppercase tracking-widest mb-1.5"
            style={{ color: 'var(--text-muted)' }}>Adjustment Type</span>
          <div className="grid grid-cols-3 gap-2" role="radiogroup" aria-labelledby="adjust-type-label">
        ...
        <Input label="Reason *" placeholder="e.g. Damaged goods, New shipment…" error={errors.reason?.message} {...register('reason')} />
    ...
    >
      <div className="space-y-2">
        {isLoading ? (
          ...
          <EmptyState icon={Activity} title="No movements recorded" description="Stock changes for this product will appear here." />
        ) : movements.map((m) => {
...
export default function InventoryPage() {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    queryKey: ['inventory', page, pageSize, debouncedSearch, stockFilter, sort.sortBy, sort.sortDir],
    queryFn: () => axiosInstance
      .get(`/products?page=${page}&limit=${pageSize}&search=${encodeURIComponent(debouncedSearch)}&stockStatus=${stockFilter}&sortBy=${sort.sortBy}&sortDir=${sort.sortDir}`)
      .then(r => r.data),
    staleTime: 30_000,
  })
    ...
    <div className="space-y-6 pb-8">
      ...
      <div className="flex gap-3 flex-wrap">
        <div className="flex items-center gap-2 px-3 h-9 rounded-lg flex-1 min-w-48"
          ...
          <input type="text" placeholder="Search products…" aria-label="Search products" value={search}
      ...
      {isLoading ? (
        ...
        <div className="rounded-xl" style={{ background: 'var(--surface-card)', border: '1px solid var(--border)' }}>
          <EmptyState
            icon={Warehouse}
            title={search || stockFilter ? 'No matching products' : 'No products found'}
            description={search || stockFilter ? 'Try a different search term or clear the status filter.' : 'Products will appear here once added to the catalog.'}
            action={(search || stockFilter) ? (
              <Button variant="secondary" size="sm" onClick={() => { setSearch(''); setStock(''); setPage(1) }}>Clear filters</Button>
            ) : undefined}
          />
        </div>
      ) : (
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-1.5">
                      <button
                        ...
                        title="View history" aria-label={`View stock history for ${p.name}`}
                      ...
                      {can('inventory_manager') && (
                        <button
                          onClick={() => setAdjust(p)}
                          title="Adjust stock" aria-label={`Adjust stock for ${p.name}`}
